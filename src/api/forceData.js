...
export const force = Array.from({ length: 100 }, (_, index) => {
    // Assurez-vous que "forces" est correctement défini
    const citiesForce = forces ? Object.keys(forces) : [];
    if (citiesForce.length === 0) return {}; // Retourne un objet vide si "forces" est invalide

    const cityForce = citiesForce[index % citiesForce.length]; // Parcours les villes de manière cyclique
    const districtsForce = forces[cityForce] ? Object.keys(forces[cityForce]) : [];
    if (districtsForce.length === 0) return {}; // Retourne un objet vide si les districts sont invalides

    const districtForce = districtsForce[index % districtsForce.length]; // Parcours les districts de manière cyclique
    const { name, address, latitude, longitude } = forces[cityForce][districtForce];

    // Retourne un objet avec les informations demandées
    return {
        name: name || 'Unknown Force',
