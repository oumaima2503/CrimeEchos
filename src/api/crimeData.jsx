...
const fetchDataFromAPI = async (latitude, longitude) => {
    try {
      ...
      const response = await fetch(`crimeData?lat=${latitude}&lng=${longitude}`);
      
      // Si vous avez des fichiers locaux ou des données à utiliser, vous pouvez aussi les charger ici
      const data = await response.json(); // Si l'API renvoie des données au format JSON
      
      // Exemple d'une réponse simulée :
      const crimes = data.crimes || [];
      const forces = data.forces || [];
      
      return { crimes, forces };
    } catch (error) {
